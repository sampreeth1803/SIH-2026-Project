...
function LiveTrackingPage({ setScreen }) {
  ...
  const load = () => fetchJson(`${API_URL}/api/cameras`).then(async (cameras) => {
    const feeds = cameras.filter((camera) => camera.source_type !== "catalogue");
    const statuses = await Promise.all(feeds.map(async (feed) => { const status = await fetchJson(`${API_URL}/api/cameras/${encodeURIComponent(feed.id)}/tracking`); return { ...feed, ...status, analytics: status.status === "Running" ? { vehicles_in_latest_frame: status.vehicle_count, processing_fps: status.processing_fps, processing_latency_ms: status.processing_latency_ms } : null, processing_ready: feed.tracking_available, original_stream_url: `/api/cameras/${encodeURIComponent(feed.id)}/stream?view=original`, tracked_stream_url: `/api/cameras/${encodeURIComponent(feed.id)}/stream?view=tracked` }; }));
    setFeeds(statuses);
  }).catch((reason) => setError(reason.message));
  useEffect(() => { load(); const timer = window.setInterval(load, 3000); return () => window.clearInterval(timer); }, []);
  const control = (feed, action) => fetchJson(`${API_URL}/api/cameras/${encodeURIComponent(feed.id)}/tracking/${action}`, { method: "POST" }).then(load).catch((reason) => setError(reason.message));
